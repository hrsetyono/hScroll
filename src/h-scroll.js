...
/*
  Animate CSS Variable when scrolling

  @param targets (Node / NodeList) - Element used as threshold on when to start/stop animating
  @param args (obj) -  scroll configuration.

  USAGE

    hScroll( document.querySelector('.my-wrapper'), {
      from: 'top-bottom',
      'to: 'bottom-top'
      '--opacity': '0 to 1'
    } )
*/
function hScroll( targets, args ) {
  args.from = args.from || 'middle-bottom'; // set default value
  args.to = args.to || 'middle-top';

  // if NodeList
  if( NodeList.prototype.isPrototypeOf( targets ) ) {
    args.direct = true; // when multiple elements is provided, it has to be direct
  ...
  /*
    Initiate hScroll animation

    @param target (Node)
  */
  function initScroll( target ) {
    let rawProps = filterProps( target );
    let props = formatProps( rawProps );
   ...
   /*
    ...
    @param target (Node) - Element used as threshold on when to start/stop animating
    @return (obj)
  */
  function filterProps( target ) {
    ...
    let keys = Object.keys( args ).filter( key => {
      return key.match(/^--/);
    });
  ...
  /*
    Format the props to be compatible with basicScroll which is { from: 'A', to: 'B' }

    @param raws (obj) - The properties in raw format (still "A to B" string)
    @return (obj)
  */
  function formatProps( raws ) {
    let formatted = {};

    for( let name in raws ) {
      if( !raws.hasOwnProperty( name ) ) { continue; }

      let timing = raws[ name ].match( /to\s\S+\s(\S+$)/ );

      try {
        formatted[ name ] = {
          from: raws[ name ].match( /(^\S+)\sto/ )[1],
          to: raws[ name ].match( /to\s(\S+)/ )[1],
          timing: timing && timing.length > 1 ? timing[1] : 'linear'
        };
      } catch( err ) {
        console.error( `hScroll Error: Wrong format in ${ name }. Should be like "A to B".` );
        console.error( err );
      }
    }
...
if( window.jQuery ) {
  // jQuery wrapper
  jQuery.fn.extend( {
    hScroll: function( args ) {
      var $targets = this;

      $targets.each( function() {
        hScroll( $(this).get(0), args );
      } );
    }
  } ); 
}